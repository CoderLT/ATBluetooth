...
DOKeyboardKeyTapAction const DOKeyboardKeyTapSpace = ^(DOKeyboard *keyboard, UIButton *key) {
...
- (BOOL)textInput:(id <UITextInput>)textInput shouldChangeCharactersInRange:(NSRange)range withString:(NSString *)string
{
    ...
    } else if ([textInput respondsToSelector:@selector(delegate)]) {
...
- (void)layoutSubviews {
    ...
    _configuration.keyboardSize = (CGSize){[[UIScreen mainScreen] bounds].size.width, 216};
    _configuration.layout(self, _keys);
}
